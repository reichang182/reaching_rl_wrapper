...
from co_design_task.context.inverse_kinematics_context_manager import InverseKinematicsContextManager
        ...
        for target_idx in range(num_targets):
            for joint_idx in range(dof):
                if num_targets == 1:
                    ...
                    joint_angles[target_idx * dof + joint_idx] = getattr(cfg, f'joint_{joint_idx}_angle')
                else:
                    # Multiple targets: use joint_i_target_j_angle
                    joint_angles[target_idx * dof + joint_idx] = getattr(cfg, f'joint_{joint_idx}_target_{target_idx}_angle')
...
def main():
    parser = argparse.ArgumentParser(description="Run WandB sweep for InverseKinematicsTask static configuration.")
    parser.add_argument("--project_name", type=str, default="ik_static_config_sweep", help="WandB project name")
    parser.add_argument("--sweep_name", type=str, default="default_static_sweep", help="WandB sweep name")
    ...
    parser.add_argument("--link_length_min", type=float, default=0.05, help="Minimum link length for sweep")
    parser.add_argument("--link_length_max", type=float, default=0.3, help="Maximum link length for sweep")
    parser.add_argument("--joint_angle_min", type=float, default=-np.pi, help="Minimum joint angle (radians)")
    parser.add_argument("--joint_angle_max", type=float, default=np.pi, help="Maximum joint angle (radians)")
    parser.add_argument("--num_trials", type=int, default=1000, help="Number of trials for the sweep")
    parser.add_argument("--machine_cost_weight", type=float, default=0.01, help="Weight for machine cost in final loss")
    parser.add_argument("--method", type=str, default="bayes", choices=["grid", "random", "bayes"], help="Sweep method")
